...
define(["N/ui/serverWidget", "N/search", "N/format", "N/file", "N/record", "../Library/slmapping.js", 'N/runtime', 'N/url', 'N/ui/message'],
    ...
    (serverWidget, search, format, file, record, slMapping, runtime, url, message) => {
        ...
        FORM.buildForm = (options) => {
            try {
                ...
                if (options.dataParam){
                    ...
                    intUser = objDataParam.user ? objDataParam.user : null
        ...
        const addFields = (options) => {
            try {
                let intUser = options.user ? options.user : [];
                // log.debug('addFields intUser', intUser);
                let arrParam = options.parameters ? options.parameters : [];
                ...
                for (var strKey in slMapping.SUITELET.form.fields) {
                    ...
                    if (slMapping.SUITELET.form.fields[strKey].ishidden) {
                        if (strKey === 'NEXT_WEEK_GRAND_TOTAL' && intUser == 1228) {
        ...
        const addSublistFields = (options) => {
            try {
                let intUser = options.user ? options.user : [];
                let arrHideFields = ['POSTING_PERIOD', 'EXCHANGE_RATE', 'NEXT_WEEK', 'NEXT_WEEK_AMOUNT', 'AGE_DAYS']
                ...
                for (var strKey in slMapping.SUITELET.form.sublistfields) {
                    ...
                    if (slMapping.SUITELET.form.sublistfields[strKey].isrolehidden) {

                        if (arrHideFields.includes(strKey) && intUser == 1228) {
                            objField.updateDisplayType({
                                displayType: serverWidget.FieldDisplayType.HIDDEN
                            });
                        } else {
                        ...
                        arrData.forEach((element, line) => {
                            ...
                            if (arrVendor.length > 0){
                                if (arrVendor.includes(vendorId)){
                                    if (element.hasOwnProperty('custpage_amount')) {
                                        element.custpage_amount_head = parseFloat(element.custpage_amount);
                                    }
                        ...
                        for (const fieldId in data) {
                            ...
                            if (validateKeys(fieldId)) {
                                ...
                                if (fieldId.includes('je')){
                                    ...
                                    let sublistFieldId;
                                    switch (fieldId) {
                                        case 'custpage_je_next_week_amount':
                                            sublistFieldId = 'custpage_next_week_amount';
                                            break;
                                        case 'custpage_je_next_week':
                                            sublistFieldId = 'custpage_next_week';
                                            break;
                                        case 'custpage_je_this_week':
                                            sublistFieldId = 'custpage_this_week';
                                            break;
                                        case 'custpage_je_this_week_amount':
                                            sublistFieldId = 'custpage_this_week_amount';
                                            break;
                                        default:
                ...
                try {
                    ...
                    let objSavedSearch = search.create({
                        ...
                        columns: [
                            ...
                            search.createColumn({ name: 'recordtype', label: 'custpage_rec_type' }),
                            ...
                            search.createColumn({ name: 'custcol_next_week_amt', label: 'custpage_je_next_week_amount'}),
                            search.createColumn({ name: 'custcol_next_week', label: 'custpage_je_next_week' }),
                            search.createColumn({ name: 'custcol_this_week', label: 'custpage_je_this_week' }),
                            search.createColumn({ name: 'custcol_this_week_amt', label: 'custpage_je_this_week_amount' }),
                        ],
                    });
                        ...
                        for (let i = 0; i < pagedData.pageRanges.length; i++) {
                            ...
                            if (pageData.length > 0) {
                                for (let pageResultIndex = 0; pageResultIndex < pageData.length; pageResultIndex++) {
                                    ...
                                    if (objData.custpage_rec_type !== "journalentry") {
                                        delete objData.custpage_lineuniquekey;
                                        delete objData.custpage_je_next_week_amount;
                                        delete objData.custpage_je_next_week;
                                        delete objData.custpage_je_this_week;
                                        delete objData.custpage_je_this_week_amount;
                                    }
