 ...
 */
            ...
            if(scriptContext.type === scriptContext.UserEventType.CREATE){
               ...
               var freightFieldArr = [];
                ...
                if(createdFrom){
                    ...
                    let arrCommodityType = ['CAPITAL', 'CONSTRUCT', 'CONSTNA']
    ...
    function afterSubmit_sr_to_inv_default_item(scriptContext){
        ...
        if(scriptContext.type === scriptContext.UserEventType.CREATE){
            ...
            if(createdFrom){
                ...
                log.debug('strCustomer', strCustomer)
                ...
                addAccessorialCode(strId, recType, strCustomer)
    ...
    const addAccessorialCode = (strId, recType, strCustomer) => {
        log.debug('START addAccessorialCode')
        const ARR_FIELDID = [
            'custbody_detention_of_trailers', 'custbody_permit_escort_charges', 'custbody_stop_off_charge', 'custbody_layover_charges_destination', 'custbody_cancelled_order_origin', 'custbody_border_crossing',
            'custbody_lift_gate_truck_or_forklift', 'custbody_redelivery', 'custbody_hazmat_charge', 'custbody_oversize_charge', 'custbody_palletize', 'custbody_beyond_point',
        ];
        let arrFieldValues = [];
        ...
        if (objRecord){
            ...
            if (arrFieldValues.length > 0) {
                for (var i = 0; i < arrFieldValues.length; i++) {
                    ...
                    for (const key in fieldValueObj) {
                        if (fieldValueObj.hasOwnProperty(key)) {
                            ...
                            let objAccessorialCode = getAccessorialCode(key, strCustomer)
                ...
                log.debug('c', recordId)
            }
        }
    }

    const getAccessorialCode = (fieldId, strCustomer) => {
        let objAccessorialCode = {}
        switch (fieldId) {
        case 'custbody_redelivery':
            objAccessorialCode.intCharges = 605;
            objAccessorialCode.strDescription = "Redelivery"
            break;
        case 'custbody_hazmat_charge':
            objAccessorialCode.intCharges = 606;
            objAccessorialCode.strDescription = "Haz-Mat Charge"
            break;
        case 'custbody_lift_gate_truck_or_forklift':
            if(strCustomer.includes('INTEL')){
                objAccessorialCode.intCharges = 127;
                objAccessorialCode.strDescription = "LIFT GATE (TRUCK) OR FORKLIFT SERVICE"
            } else if(strCustomer.includes('CISCO')){
                objAccessorialCode.intCharges = 607;
                objAccessorialCode.strDescription = "LiftGate"
            } else {
                objAccessorialCode.intCharges = 607;
                objAccessorialCode.strDescription = "LiftGate"
            }
            break;
        case 'custbody_oversize_charge':
            objAccessorialCode.intCharges = 608;
            objAccessorialCode.strDescription = "Over-Size Charge"
            break;
        case 'custbody_palletize':
            objAccessorialCode.intCharges = 609;
            objAccessorialCode.strDescription = "Palletize"
            break;
        case 'custbody_beyond_point':
            objAccessorialCode.intCharges = 610;
            objAccessorialCode.strDescription = "Beyond Point"
            break;
        case 'custbody_border_crossing':
            objAccessorialCode.intCharges = 595;
            objAccessorialCode.strDescription = "Border Crossing"
            break;
